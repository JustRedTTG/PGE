...
class Sheet:
    def __init__(self, file, sheet_handler:sheet_handler):
        self.surface = Surface(surface=pygame.image.load(file).convert_alpha())
        sheet_handler.map(self.surface)
        self.handler = sheet_handler
...
class Sprite:
    def __init__(self, sprite_reference:[Sheet, str], scale=None, pos:tuple=(0, 0), name="Sprite", layer=0):
        if isinstance(sprite_reference, Sheet):
            ...
            if scale: self.reference.resize(scale)
    ...
    def skip_frame(self, speed:int=None):
        ...
        if self.multiplier > 0 and self.index >= self.reference.frames-1: # Check direction and index
            if self.pong: self.multiplier = -1 # Flip the pong
            elif self.index > self.reference.frames: self.index = 0 # Reset frame index
        elif self.multiplier < 0 and self.index <= 0:  # Check reverse direction and reverse index
            if self.pong: self.multiplier = 1 # Flip the pong
            elif self.index < 0: self.index = self.reference.frames # Reset frame index
