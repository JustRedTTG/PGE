...
def touching_perimeter_right(rects, r):
    return sum(max(0, min(pr.bottom, r.bottom) - max(pr.y, r.y)) for pr in rects if pr.right == r.right or pr.x == r.right)


def touching_perimeter_top(rects, r):
    return sum(max(0, min(pr.right, r.right) - max(pr.x, r.x)) for pr in rects if pr.bottom == r.bottom or pr.y == r.bottom)
